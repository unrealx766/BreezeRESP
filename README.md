...
BreezeRESP 是一款基于 Tauri v2 构建的原生桌面 Redis 客户端。相比 Electron 方案，安装包体积更小、内存占用更低、启动速度更快。提供从连接管理到数据浏览、实时监控、批量执行、沙箱预演的完整 Redis 操作体验，支持中英文界面。
...
启动后进入连接页面，点击「新建连接」，填写名称、主机、端口，按需开启密码和 SSL，点击「测试连接」确认可用后保存。点击卡片上的「连接」按钮即可连接并进入数据浏览器。

### 浏览与编辑数据

左侧树形面板按 `:` 分隔符自动折叠键层级，点击展开查看子键。选中任意 Key 后中间面板展示详情，支持双击内联编辑键名、字段值、成员、分数。右侧面板显示 TTL 仪表盘与编码信息。顶部可切换 DB、搜索 Key、按类型筛选。

### 执行 Pipeline

切换到 Pipeline 页面，逐条添加 Redis 命令，拖拽调整顺序，点击执行。执行完成后查看每条命令的结果与总延迟统计。可将常用命令组合保存为脚本，下次直接加载使用。

### 沙箱预演

切换到沙箱页面，输入任意 Redis 命令，点击预览查看执行效果与数据变更 Diff。确认无误后执行，如有误可通过快照一键回滚。
