...
- **连接管理** — 支持多连接配置，SSL/TLS 加密连接，本地加密存储密码
- **数据浏览** — 树形 / 列表双视图切换，级联懒加载，支持 String / Hash / List / Set / ZSet 五种数据类型
- **实时监控** — QPS 趋势图、内存用量、命中率、CPU 使用等关键指标仪表盘
- **Pipeline 模式** — 批量命令编排，一键执行，延迟统计
- **沙箱模式** — 命令预演 Diff 预览，支持快照与一键回滚，安全操作不翻车
- **国际化** — 内置中文 / English 双语支持
